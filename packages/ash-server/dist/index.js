...
__export(index_exports, {
  ASH_ERROR_HTTP_STATUS: () => import_ash_core4.ASH_ERROR_HTTP_STATUS,
  ASH_ERROR_MESSAGES: () => import_ash_core4.ASH_ERROR_MESSAGES,
  AshError: () => import_ash_core4.AshError,
  CanonicalizationError: () => import_ash_core4.CanonicalizationError,
  ContextExpiredError: () => import_ash_core4.ContextExpiredError,
  EndpointMismatchError: () => import_ash_core4.EndpointMismatchError,
  IntegrityFailedError: () => import_ash_core4.IntegrityFailedError,
  InvalidContextError: () => import_ash_core4.InvalidContextError,
  MemoryContextStore: () => MemoryContextStore,
  RedisContextStore: () => RedisContextStore,
  ReplayDetectedError: () => import_ash_core4.ReplayDetectedError,
  SqlContextStore: () => SqlContextStore,
  UnsupportedContentTypeError: () => import_ash_core4.UnsupportedContentTypeError,
  ashErrorHandler: () => ashErrorHandler,
  ashMiddleware: () => ashMiddleware,
  ashPlugin: () => ashPlugin,
...
var import_ash_core4 = require("@anthropic/ash-core");
...
async function verifyRequest(store, req, options) {
  ...
  if (typeof contextId !== "string" || contextId === "") {
    throw new import_ash_core.InvalidContextError("Missing or invalid context ID");
  }
  const context = await store.get(contextId);
  if (context === null) {
    throw new import_ash_core.InvalidContextError();
  }
  const now = Date.now();
  if (context.expiresAt <= now) {
    throw new import_ash_core.ContextExpiredError();
  }
  if (context.binding !== expectedBinding) {
    throw new import_ash_core.EndpointMismatchError();
  }
  const payload = extractPayload(req);
  const canonicalPayload = canonicalizePayload(payload, contentType);
  const expectedProof = (0, import_ash_core.buildProof)({
    mode: context.mode,
  ...
  if (typeof providedProof !== "string" || !(0, import_ash_core.timingSafeCompare)(expectedProof, providedProof)) {
    throw new import_ash_core.IntegrityFailedError();
  }
  const consumeResult = await store.consume(contextId, now);
  if (consumeResult === "already_consumed") {
    throw new import_ash_core.ReplayDetectedError();
  }
...
function canonicalizePayload(payload, contentType) {
  switch (contentType) {
    case "application/json":
      return (0, import_ash_core.canonicalizeJson)(payload);
    case "application/x-www-form-urlencoded":
      if (typeof payload === "string") {
        return (0, import_ash_core.canonicalizeUrlEncoded)(payload);
      }
      if (typeof payload === "object" && payload !== null) {
        return (0, import_ash_core.canonicalizeUrlEncoded)(payload);
      }
      throw new import_ash_core.UnsupportedContentTypeError("Invalid payload for URL-encoded content type");
    default:
      throw new import_ash_core.UnsupportedContentTypeError(`Content type not supported: ${contentType}`);
  }
}
...
var import_ash_core2 = require("@anthropic/ash-core");
...
function ashMiddleware(store, options) {
  ...
  return async (req, res, next) => {
    try {
      ...
      if (error instanceof import_ash_core2.AshError) {
...
function ashErrorHandler() {
  return (err, _req, res, next) => {
    if (err instanceof import_ash_core2.AshError) {
...
var import_ash_core3 = require("@anthropic/ash-core");
  ...
  fastify.decorate(
    ...
    (verifyOptions) => async (request, reply) => {
      ...
      try {
        ...
        if (error instanceof import_ash_core3.AshError) {
